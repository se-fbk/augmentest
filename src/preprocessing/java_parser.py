...
class JavaClassParser():
    ...
    def get_preceding_comments(function_node, blob: str):
        ...
        if current_node.type in ("line_comment", "block_comment", "documentation_comment"):
    ...
    def get_function_comments(function_node, blob: str):
        ...
        for block_node in (node for node in function_node.children if node.type == 'block'):
            for block_node_child in (node for node in block_node.children if node.type in ("line_comment", "block_comment", "documentation_comment")):
    ...
    # use tree sitter to get the class name
    def get_class_name(self, content):
        tree = self.parser.parse(bytes(content, "utf8"))
        for node in tree.root_node.children:
            if node.type == 'class_declaration':
                # Find the child node that represents the class name
