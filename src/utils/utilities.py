...
from src.config.config import get_grammar, class_info_output, get_language, get_extension
from src.execution.test_runner import TestRunner
from src.preprocessing.java_parser import JavaClassParser


class ProjectUtilities:

    @staticmethod
    def compile_test_suite(test_path, target_path, test_filename, package, class_name):
        test_compiler = TestOrchestrator(test_path, target_path, test_filename, package, class_name)
        return test_compiler.compile_test_suite()

    @staticmethod
    def compile_project(project_root):
        test_compiler = TestOrchestrator(None, None, None, None, None)
        return test_compiler.compile_project(project_root)

    @staticmethod
    def execute_test_suite(test_path, target_path, test_filename, package, class_name):
        test_executor = TestOrchestrator(test_path, target_path, test_filename, package, class_name)
...
class TestOrchestrator:
    ...
    def compile_test_suite(self):
        return self.runner.compile_test()

    def compile_project(self, project_root):
        return self.runner.compile_with_maven(project_root)
